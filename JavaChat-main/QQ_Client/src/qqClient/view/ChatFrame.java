...
public class ChatFrame extends JFrame {
    ...
    public ChatFrame(String userId, String getterId) {
	 ...
	 /* 发送文件 */
        // 发送文件 内容框 
        txt_SendFile.setBounds(10, 360, 400, 32);
        this.add(txt_SendFile);

        // 文件选择按钮  调出文件选择器
        btn_ChooseFile.setBounds(420, 360, 100, 30);
        btn_ChooseFile.setText("选择文件");
        this.add(btn_ChooseFile);
        btn_ChooseFile.setFont(new Font("黑体", Font.BOLD, 14));

        // 发送文件 按钮
        btn_SendFile.setBounds(530, 360, 100, 30);
	...
	/* 消息输入 */
        ...
        txt_Send.setBorder(BorderFactory.createLineBorder(Color.BLACK));
        this.add(txt_Send);                   // 聊天输入面板
        
        
    /* 消息发送 */
        // 发送消息 按钮图片
		ImageIcon ico_login = new ImageIcon("image/PiKaQ.png");
		ico_login.setImage(ico_login.getImage().getScaledInstance(280, 40,
				Image.SCALE_DEFAULT));	
		// 设置 发送消息按钮
        ...
        btn_Send.setBounds(10, 510, 280, 40); // 添加 发送消息 按钮
        this.add(btn_Send);
        btn_Send.setFont(new Font("黑体", Font.BOLD, 16));
