...
public class LoginFrame extends JFrame implements ActionListener {
	...
	/* 顶部logo栏 */
		// 插入顶部logo
		ImageIcon logo = new ImageIcon("image/OIP-C.png");
		logo.setImage(logo.getImage().getScaledInstance(800, 250,
				Image.SCALE_DEFAULT));
	...
	/* 中部左侧 头像栏 */
		// 中间右侧 头像图片
		ImageIcon headIcon = new ImageIcon("image/touxiang.png");
		headIcon.setImage(headIcon.getImage().getScaledInstance(200, 250,
				Image.SCALE_DEFAULT));
	...
	/* 定义中间右侧 输入栏组件 */
		...
		txt_user.setPreferredSize(new Dimension(200,30));
		txt_pwd = new JPasswordField();
		txt_pwd.setPreferredSize(new Dimension(200,30));
		
		// 清除号码 和 注册账号按钮
		btn_sweep = new JButton("清除号码");
		btn_sweep.setFont(new Font("黑体", Font.PLAIN, 16));
